<script lang="ts">
	...
	let activeTab = 'friends'; // 'friends', 'requests', 'sent', 'search'
	let friends: Friend[] = [];
	...
	function switchTab(tab: string) {
		...
		if (tab === 'friends' && friends.length === 0) {
			fetchFriends();
		} else if (tab === 'requests' && receivedRequests.length === 0) {
...
<div class="flex h-screen overflow-hidden bg-white">
	<!-- Main Content -->
	<div class="flex-1 flex flex-col overflow-hidden max-w-xl mx-auto w-full">
		<!-- Header -->
		<div class="px-6 py-5 border-b border-gray-200 bg-white">
			<h1 class="text-xl font-semibold text-gray-900 mb-4">Friends</h1>
				...
				<div class="relative">
					<input
						...
						class="w-full px-4 py-2 bg-gray-50 rounded-md text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-gray-300 focus:bg-white transition-all pr-10"
					/>
					<svg
				...
				<!-- Tabs -->
				<div class="flex gap-4 mt-5 -mb-[1px]">
					<button
						on:click={() => switchTab('friends')}
						class="pb-2 px-1 text-sm font-medium transition-colors {activeTab === 'friends' ? 'text-gray-900' : 'text-gray-500 hover:text-gray-700'}"
					>
						Friends {friends.length > 0 ? `(${friends.length})` : ''}
					</button>
					<button
						on:click={() => switchTab('requests')}
						class="pb-2 px-1 text-sm font-medium transition-colors {activeTab === 'requests' ? 'text-gray-900' : 'text-gray-500 hover:text-gray-700'}"
					>
						Requests
						{#if receivedRequests.length > 0}
							<span class="ml-1 text-gray-500">({receivedRequests.length})</span>
						{/if}
					</button>
					<button
						on:click={() => switchTab('sent')}
						class="pb-2 px-1 text-sm font-medium transition-colors {activeTab === 'sent' ? 'text-gray-900' : 'text-gray-500 hover:text-gray-700'}"
					>
		...
		<div class="flex-1 overflow-y-auto px-6 py-4">
			...
			{#if searchQuery.length >= 2}
				<div class="mb-6">
					<h2 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3 px-1">
						Search Results
					</h2>
										...
										<div class="flex-shrink-0">
											{#if user.status === 'none'}
												<button
													...
													class="px-4 py-1.5 bg-blue-500 text-white text-xs font-medium rounded-md hover:bg-blue-600 transition-colors"
												>
													Add Friend
												</button>
											{:else}
												...
												{#if badge}
													<div class="px-3 py-1.5 {badge.color} text-xs font-medium rounded-md">
														{badge.text}
													</div>
												{/if}
											{/if}
								...
								<div class="flex items-center gap-3">
									...
									<div class="flex-1 min-w-0">
										...
										<p class="text-xs text-gray-500">
											{friend.is_online ? 'Online' : friend.last_seen ? `Last seen ${formatDate(friend.last_seen)}` : 'Offline'}
										</p>
									</div>
									<div class="flex items-center gap-2 flex-shrink-0">
										<a
											href="/messenger?user={friend.id}"
											class="px-4 py-1.5 bg-blue-500 text-white text-xs font-medium rounded-md hover:bg-blue-600 transition-colors"
										>
											Message
										</a>
							...
							<div class="bg-white border border-gray-200 rounded-lg p-3 hover:bg-gray-50 transition-colors">
								<div class="flex items-center gap-3">
									...
									<div class="flex gap-2 flex-shrink-0">
										<button
											on:click={() => respondToRequest(request.friendship_id, 'accept')}
											class="px-4 py-1.5 bg-green-500 text-white text-xs font-medium rounded-md hover:bg-green-600 transition-colors"
										>
											Accept
										</button>
										<button
											on:click={() => respondToRequest(request.friendship_id, 'reject')}
						...
						{#each sentRequests as request (request.friendship_id)}
							<div class="bg-white border border-gray-200 rounded-lg p-3 hover:bg-gray-50 transition-colors">
								<div class="flex items-center gap-3">
									...
									<button
										on:click={() => cancelFriendRequest(request.friendship_id)}
										class="px-4 py-1.5 bg-gray-100 text-gray-700 text-xs font-medium rounded-md hover:bg-gray-200 transition-colors flex-shrink-0"
									>
										Cancel
									</button>
								</div>
